...
/**
 * @author walsli the class is made of several base methods which are added onto
...
public class BoxGrabber {
	...
	public enum State {
		...
		LIFTUPAUTOMATED, LIFTDOWNAUTOMATED,

		SWITCHDOWNAUTOMATED,

		MANUALCONTROL, STOPOVERRIDE
	...
	/**
	 * this is for full on manual control with the x-box d pad based on the unit
	 * circul math
	 */
	...
	/**
	 * @return status of hardware systems on the robot when called to shufle boarad.
	 */
	...
	/**
	 * @return robot action iniatied from xbox controller at drivers station the
	 *         state will be called by the init function and overides any actionn
	 *         running on the robot their are no time interupts on the robot ever
	 *         used.
	 */
	public void boxGrabberPeriodic() {
		routineRunning = liftUpActivated || liftDownActivated || isKickoutActivated || liftUpSwitchActivated
				|| liftDownSwitchActivated || liftUpLowScaleActivated;
		...
		if (!routineRunning || _xBox.getPOV() != -1 || _xBox.getBumper(GenericHID.Hand.kLeft)
				|| _xBox.getBumper(GenericHID.Hand.kRight) || _xBox.getBackButton() || _xBox.getStartButton()) {
			switch (xBox()) {
			case STOPOVERRIDE:
				stop();
				liftUpActivated = false;
