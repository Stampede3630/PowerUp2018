...
public class Robot extends IterativeRobot {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.wpi.first.wpilibj.IterativeRobotBase#testInit() init method for test
	 * modde
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.wpi.first.wpilibj.IterativeRobotBase#autonomousPeriodic() irative
	 * method for auto. calls all methods during auto that are used
	 */
	/*
	 * (non-Javadoc)
	 * 
	 * @see edu.wpi.first.wpilibj.IterativeRobotBase#autonomousPeriodic()u calls
	 * periodic methods used during auto
	 */
	@Override
	...
	public void autoLogic() {
		if (gameData.length() > 0) {
			...
			if ((gameData.charAt(0) == 'L') && (gameData.charAt(1) == 'L')) {
				...
				} else if ((autoChooser.getSelected() == StartingPoints.LEFT)
						&& (autoLLL.getSelected() == Destinations.SCL)) {
					_driveTrain.leftScaleLeft();
				} else if ((autoChooser.getSelected() == StartingPoints.RIGHT)
						&& (autoLLL.getSelected() == Destinations.SWL)) {
					_driveTrain.rightSwitchLeft();
				} else if ((autoChooser.getSelected() == StartingPoints.RIGHT)
						&& (autoLLL.getSelected() == Destinations.SCL)) {
					_driveTrain.rightScaleLeft();
				} else if ((autoChooser.getSelected() == StartingPoints.MIDDLE)
						&& (autoLLL.getSelected() == Destinations.SWL)) {
				...
				if ((autoChooser.getSelected() == StartingPoints.LEFT) && (autoRRR.getSelected() == Destinations.SWR)) {
					_driveTrain.leftSwitchRight();
						...
						&& (autoRRR.getSelected() == Destinations.SCR)) {
					_driveTrain.leftScaleRight();
				} else if ((autoChooser.getSelected() == StartingPoints.RIGHT)
						&& (autoRRR.getSelected() == Destinations.SWR)) {
				...
				if ((autoChooser.getSelected() == StartingPoints.LEFT) && (autoLRL.getSelected() == Destinations.SWL)) {
					_driveTrain.leftSwitchLeftFF();
						...
						&& (autoLRL.getSelected() == Destinations.SCR)) {
					_driveTrain.leftScaleRight();
				} else if ((autoChooser.getSelected() == StartingPoints.RIGHT)
						&& (autoLRL.getSelected() == Destinations.SWL)) {
					_driveTrain.rightSwitchLeft();
				} else if ((autoChooser.getSelected() == StartingPoints.RIGHT)
						&& (autoLRL.getSelected() == Destinations.SCR)) {
					_driveTrain.rightScaleRight();
				...
				if ((autoChooser.getSelected() == StartingPoints.LEFT) && (autoRLR.getSelected() == Destinations.SWR)) {
					_driveTrain.leftSwitchRight();
						...
						&& (autoRLR.getSelected() == Destinations.SCL)) {
					_driveTrain.leftScaleLeft();
				} else if ((autoChooser.getSelected() == StartingPoints.RIGHT)
						&& (autoRLR.getSelected() == Destinations.SWR)) {
					_driveTrain.rightSwitchRightFF();
				} else if ((autoChooser.getSelected() == StartingPoints.RIGHT)
						&& (autoRLR.getSelected() == Destinations.SCL)) {
					_driveTrain.rightScaleLeft();
