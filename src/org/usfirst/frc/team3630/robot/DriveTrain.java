...
public class DriveTrain {
	...
	public DriveTrain(BoxGrabber _boxGrabber) {
		...
		turnController = new PIDController(Consts.kPRotAng, Consts.kIRotAng, Consts.kDRotAng, ahrs,
				new MyRotationPidOutput());
		...
		posController = new PIDController(Consts.kPPos, Consts.kIPos, Consts.kDPos, positionEncoderSource,
				new MyPosPidOutput());
	...
	/**
	 * dose this method work seems to have comented out unfinshed pices of code?
	 * just want to double chack if something needs to be finished? or is missing
	 */
	...
	private class EncoderPIDSource implements PIDSource {
		...
		public double pidGet() {
			...
			if (right) {
				...
				positionInches = fRGetSelected * (double) (2 * Math.PI * Consts.wheelRadiusInch)
						/ (double) Consts.ticksPerRotation;
				SmartDashboard.putString("Right", "Right calling");
			} else {
				positionInches = fLGetSelected * (double) (2 * Math.PI * Consts.wheelRadiusInch)
						/ (double) Consts.ticksPerRotation;
